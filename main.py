...
from medicare_tools.helpers import create_directory, get_files_in_dir, delete_files_in_dir
from medicare_tools.logger import setup_logger
from medicare_tools.queries import get_notes_log_stmt, get_time_log_stmt, get_fulfillment_stmt, get_patient_id_stmt, \
...
class DataImporter:
    def __init__(self, start_date, end_date, logger=None):
        ...
        self.gps.create_engine(username=os.getenv('GPS_SQL_USERNAME'),
...
def import_all_data(start_date, end_date, logger=logging.getLogger()):
    ...
    gps.create_engine(username=os.getenv('GPS_SQL_USERNAME'),
                      password=os.getenv('GPS_SQL_PASSWORD'),
                      host=os.getenv('GPS_SQL_HOST'),
                      database=os.getenv('GPS_SQL_DB'))
