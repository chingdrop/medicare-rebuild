...
`call_time_seconds` is an `INTEGER` column, so `SUM(call_time_seconds) / 60` in the 99202 and 99457 conditions is integer division; the scenarios sit on whole-minute boundaries, where integer and decimal division agree regardless.
