...
| SQL Server-specific | The schema is now SQLAlchemy declarative models, which are more portable in principle than hand-written T-SQL, but the billing procedures still use T-SQL syntax (`DATEADD`, `CROSS JOIN`, `DBCC CHECKIDENT`), and `DatabaseManager` connects through `mssql+pyodbc` and ODBC Driver 18 specifically. Porting to another RDBMS would still need rework, not a config change. | [sql/stored_procedures/](../sql/stored_procedures/); [models.py](../src/medicare_rebuild/models.py); [db_utils.py](../src/medicare_rebuild/utils/db_utils.py) |
