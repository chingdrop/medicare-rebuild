...
- The REST adapter (`utils/rest_adapter.py`, copied from the `py-shared-tools` library) logs request parameters and bodies at debug level, and the Microsoft token request body contains the Azure AD client secret. `api_utils.py` therefore gives the adapter a child logger held at INFO (`_http_logger`), so those request lines are not emitted even though `main()` runs at debug level. Adapter records at INFO and above still reach the pipeline's handlers.
