...
| [0007](0007-sqlalchemy-engine-and-raw-sql.md) | Use SQLAlchemy as an engine and session layer with raw SQL, not ORM models | *Superseded by 0015.* SQLAlchemy engines and sessions with raw SQL and pandas I/O; no ORM models. |
...
| [0016](0016-alembic-migrations-for-the-gps-database.md) | Manage the GPS schema with Alembic migrations | Alembic, targeting `GpsBase.metadata` only, gives a real GPS deployment versioned, incremental migrations; the demo and integration tests keep using `create_all()`. |
