...
def create_patient_status_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    patient_status_df['temp_user'] = 'svc_helpdesk'
...
def standardize_patient_notes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df.loc[df['AZURE_UPN'].isin(['Joycelynn Harris']), 'Recording_Time'] = 900
    ...
    df.loc[df['AZURE_UPN'].isin(['Joycelynn Harris', 'Melanie Coffey', 'Krista Lewin']), 'Time_Note'] = 'Initial Evaluation'
    df.loc[df['AZURE_UPN'].isin(['Christylyn Diosma', 'Maria Albingco', 'Mary Cortes', 'Richel Rodriguez', 'Rigel Sagayno']), 'Time_Note'] = 'Alert'
