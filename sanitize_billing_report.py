...
def fill_primary_payer(row):
    if pd.isnull(row['Primary Payer']) and pd.isnull(row['Primary Payer ID']):
        return 'Medicare Part B'
    return row['Primary Payer']

def fill_primary_payer_id(row):
    if row['Primary Payer'] == 'Medicare Part B' and pd.isnull(row['Primary Payer ID']) and not pd.isnull(row['MedicareNumber']):
...
# report_df['Primary Payer'] = report_df['Primary Payer'].fillna('Medicare Part B')
# report_df['Primary Payer ID'] = report_df['Primary Payer ID'].fillna(report_df['MedicareNumber'])
